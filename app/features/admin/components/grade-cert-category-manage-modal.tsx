...
import type { GradeCertCategory } from "~/features/support/schema";
...
type Props = {
  ...
  categories: GradeCertCategory[];
  onSubmitCategory: (intent: string, fields: Record<string, string>) => void;
};

export function GradeCertCategoryManageModal({
  open,
  onOpenChange,
  categories,
  onSubmitCategory,
  ...
  const startEdit = (c: GradeCertCategory) => {
  ...
  const handleDelete = (c: GradeCertCategory) => {
    if (c.name === PROTECTED_NAME) return;
    if (!confirm(`「${c.name}」 카테고리를 삭제할까요?\n(이 카테고리를 쓰는 등급판정서가 있으면 삭제되지 않습니다.)`)) return;
