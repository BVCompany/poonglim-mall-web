...
import type { NewsCategory } from "~/features/media/schema";
...
export function NewsCategoryManageModal({
  open,
  onOpenChange,
  categories,
  onSubmitCategory,
  ...
  const startEdit = (c: NewsCategory) => {
  ...
  const handleDelete = (c: NewsCategory) => {
    if (c.name === PROTECTED_NAME) return;
    if (!confirm(`「${c.name}」 카테고리를 삭제할까요?\n(이 카테고리를 쓰는 보도자료가 있으면 삭제되지 않습니다.)`)) return;
