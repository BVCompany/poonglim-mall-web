...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const [dbNews, dbNewsCategories] = await Promise.all([
    ...
    db
      .select()
      .from(newsCategories)
      .orderBy(asc(newsCategories.sort_order))
      .catch(() => []),
  ]);
...
export async function action({ request }: Route.ActionArgs) {
  await requireAdminMutation(request, ADMIN_PERMISSIONS.NEWS, "news");
  ...
  if (intent === "create") {
    ...
    if (locale === "en" && wantsFeatured) {
      return {
        success: false as const,
        error: "featured_ko_only" as const,
        intent: "create" as const,
      };
    }
  ...
  if (intent === "create_translation") {
    ...
    const [exists] = await db
      ...
      .where(
        and(
          eq(news.translation_group_id, base.translation_group_id),
          eq(news.locale, targetLocale),
        ),
      )
      .limit(1);
    if (exists)
  ...
  if (intent === "update") {
    ...
    if (id) {
      ...
      if (
        wantsFeatured &&
        !wasFeatured &&
        (await countFeaturedExcluding(id)) >= MAX_FEATURED
      ) {
        return {
          success: false as const,
          error: "max_featured" as const,
  ...
  if (intent === "toggle_featured") {
    ...
    const [row] = await db
      ...
      .limit(1);
    if (row?.locale === "en")
      return { success: false as const, error: "featured_ko_only" as const };
  ...
  if (intent === "category_create") {
    ...
    const [mx] = await db
      .select({
        v: sql<number>`COALESCE(MAX(${newsCategories.sort_order}), -1)`,
      })
      .from(newsCategories);
  ...
  if (intent === "category_update") {
    ...
    if (
      row.name === PROTECTED_NEWS_CATEGORY &&
      newName !== PROTECTED_NEWS_CATEGORY
    ) {
...
export default function AdminMediaNewsPage({
  loaderData,
  ...
  const featuredCount = useMemo(
    () => dbNews.filter((n) => n.is_featured).length,
    [dbNews],
  );
  ...
  useEffect(() => {
    ...
    if ("error" in fetcher.data && fetcher.data.error === "max_featured") {
      window.alert(
        `주요 보도는 최대 ${MAX_FEATURED}건까지 지정할 수 있습니다.`,
      );
      return;
    }
    ...
    if (
      "error" in fetcher.data &&
      fetcher.data.error === "category_protected"
    ) {
      window.alert(
        `「${PROTECTED_NEWS_CATEGORY}」 카테고리는 삭제하거나 이름을 바꿀 수 없습니다.`,
      );
      return;
    }
    if ("error" in fetcher.data && fetcher.data.error === "category_in_use") {
      window.alert(
        "이 카테고리를 사용 중인 보도자료가 있어 삭제할 수 없습니다. 먼저 해당 글의 카테고리를 변경하세요.",
      );
      return;
    }
    if (
      "error" in fetcher.data &&
      fetcher.data.error === "category_duplicate"
    ) {
    ...
    if (
      "error" in fetcher.data &&
      fetcher.data.error === "category_validation"
    ) {
    ...
    if (
      ...
      if (fetcher.data.intent === "create_translation") {
        window.alert(
          "영문 초안이 추가되었습니다. 목록에서 해당 글을 열어 내용을 입력해 주세요.",
        );
      }
  ...
  const appendFormFields = (fd: FormData) => {
    ...
    fd.append(
      "body_image_urls",
      JSON.stringify(form.body_image_urls.filter(Boolean)),
    );
  ...
  const handleOpenEdit = (item: (typeof dbNews)[number]) => {
    ...
    setForm({
      ...
      published_at:
        toInputDate(item.published_at) || new Date().toISOString().slice(0, 10),
          ...
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-gray-600">
                ...
                <span className="font-medium text-[#003F2B]">
                  주요 보도: {featuredCount}건
                </span>
                <span className="text-gray-500">
                  {" "}
                  (홈 보도자료 상단 슬라이더 최대 {MAX_FEATURED}건)
                </span>
              </p>
              <p className="mt-1 text-xs text-gray-500">
                총 {dbNews.length}건 · 목록 {filtered.length}건
              </p>
            </div>
            <Button
              type="button"
                        ...
                        <span
                          className={cn(
                            ...
                            newsCategoryBadgeClass(
                              categoryColorByName.get(item.type) ?? "slate",
                            ),
                          )}
                        >
                          {item.type}
                    ...
                    <div className="flex shrink-0 flex-col items-center gap-0.5 md:gap-1">
                      <Button
                        ...
                        className={cn(
                          ...
                          "text-gray-500 hover:bg-[#FDFDF5] hover:text-[#02633E]",
                          item.is_featured &&
                        ...
                        onClick={() =>
                          handleToggleFeatured(item.news_id, item.is_featured)
                        }
                      >
                        <Star
                          className={cn(
                      ...
                      {item.locale === "ko" ? (
                        <Button
                          ...
                          title="영문 버전 초안 추가"
                          onClick={() =>
                            handleAddTranslation(item.news_id, "en")
                          }
                        >
                          EN 추가
                        </Button>
                      ...
                      <Button
                        ...
                        size="icon"
                        variant="ghost"
                        className="rounded-lg border border-gray-200 bg-white text-gray-500 shadow-sm transition-all hover:border-[#02633E]/45 hover:bg-[#FDFDF5] hover:text-gray-900 hover:shadow-md"
                        title="수정"
                      ...
                      <Button
                        ...
                        size="icon"
                        variant="ghost"
                        className="rounded-lg border border-gray-200 bg-white text-gray-500 shadow-sm transition-all hover:border-red-300 hover:bg-red-50/60 hover:text-red-600 hover:shadow-md"
                        title="삭제"
          ...
          <form onSubmit={handleAdd} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <select
                value={form.locale}
                onChange={(e) =>
                  setForm({ ...form, locale: e.target.value as "ko" | "en" })
                }
              ...
              <p className="text-xs text-gray-500">
                영문은 별도 행으로 저장되며, 이후 「EN 추가」로 묶을 수도
                있습니다.
              </p>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <DatePicker
                  value={
                    form.published_at
                      ? new Date(`${form.published_at}T12:00:00`)
                      : undefined
                  }
                  onChange={(d) =>
                    setForm({
            ...
            <div className="flex items-start justify-between gap-4 rounded-xl border border-gray-100 bg-gray-50/90 px-4 py-3">
              <div className="min-w-0 space-y-0.5">
                <Label
                  htmlFor="add-featured-switch"
                  className="text-sm font-medium text-gray-900"
                >
                  주요 보도로 설정
                </Label>
                <p className="text-xs text-gray-500">
                  메인 페이지 상단 슬라이더에 노출됩니다
                </p>
              </div>
              <Switch
            ...
            <div className="space-y-2">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  보도자료를 이미지로 올릴 경우 여기에 추가하세요. 여러 장 등록
                  가능합니다.
                </p>
              </div>
              <MultiImageUpload
