...
export async function action({ request }: Route.ActionArgs) {
  await requireAdminMutation(request, ADMIN_PERMISSIONS.SITE_HOME, "site_home");
  ...
  if (intent === "save_company_intro") {
    await Promise.all([
      upsertSetting(
        SETTING_KEYS.COMPANY_INTRO_IMAGE,
        (fd.get("image") as string) ?? "",
      ),
      upsertSetting(
        SETTING_KEYS.COMPANY_INTRO_TITLE,
        (fd.get("title") as string) ?? "",
      ),
      upsertSetting(
        SETTING_KEYS.COMPANY_INTRO_LINK,
        (fd.get("link") as string) ?? "",
      ),
    ]);
    return { success: true };
  }
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  <Building2 className="h-5 w-5 text-[#204E3A]" />
                  <CardTitle>회사소개 배너</CardTitle>
                </div>
                <CardDescription>
                  홈 화면 중간에 표시되는 회사 소개 섹션의 이미지와 텍스트를
                  관리합니다.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSaveCompanyIntro} className="space-y-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <ImageUpload
                      ...
                      onChange={(url) =>
                        setCompanyIntro({ ...companyIntro, image: url })
                      }
                      aspectRatio="16/6"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-gray-400">
                      배너 위에 표시될 메인 문구입니다. 비워두면 기본 문구가
                      표시됩니다.
                    </p>
                  </div>
