...
import { CertificationAddModal, type CertFormData } from "../components/certification-add-modal";
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "update") {
    ...
    if (id) {
      await db.update(brandCertItems).set({
        type:        (fd.get("type") as "award" | "cert") ?? "cert",
        title:       fd.get("title") as string,
        year:        (fd.get("year") as string) || null,
        description: (fd.get("description") as string) || null,
  ...
  if (intent === "toggle_active") {
    ...
    if (id) {
      await db.update(brandCertItems)
...
export default function AdminCertificationsScreen({ loaderData }: Route.ComponentProps) {
  ...
  const submitItem = (data: CertFormData, intent: "create" | "update", id?: number) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white">
                    ...
                    filtered.map((item, idx) => (
                      ...
                      >
                        <td className="px-5 py-3">
                          <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${TYPE_STYLE[item.type] ?? "bg-gray-100 text-gray-600"}`}>
                        ...
                        <td className="px-5 py-3 text-center text-gray-500">{item.sort_order}</td>
                        <td className="px-5 py-3 text-center">
                          <Button
                            variant="ghost"
                            size="sm"
      ...
      <CertificationAddModal
        ...
        onSubmit={(data) => { submitItem(data, "create"); setAddOpen(false); }}
      />
      ...
      {editingId !== undefined && (
        <CertificationAddModal
          ...
          onSubmit={(data) => { submitItem(data, "update", editingId); setEditingId(undefined); }}
          editId={editingId}
