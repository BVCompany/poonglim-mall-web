...
export async function loader({ request }: Route.LoaderArgs) {
  const adminUser = await requireAdminPermission(
    request,
    ADMIN_PERMISSIONS.PRODUCT_CATEGORIES,
  );
...
export async function action({ request }: Route.ActionArgs) {
  await requireAdminMutation(request, ADMIN_PERMISSIONS.PRODUCT_CATEGORIES, "product_categories");
  const fd = await request.formData();
  const intent = fd.get("intent") as string;

  if (intent === "create") {
    const maxOrder = await db
      .select({ sort_order: productCategories.sort_order })
  ...
  if (intent === "reorder") {
    ...
    if (idx >= 0 && swapIdx >= 0 && swapIdx < all.length) {
      ...
      await db
        ...
        .where(eq(productCategories.category_id, a.category_id));
      await db
        .update(productCategories)
        .set({ sort_order: aOrder })
        .where(eq(productCategories.category_id, b.category_id));
    }
    return { success: true };
  }
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="예: Liquid Egg Products"
            />
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-8">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (
                                !confirm(
                                  `"${cat.name}" 카테고리를 삭제하시겠습니까?\n해당 카테고리의 제품이 있다면 카테고리 미분류 상태가 됩니다.`,
                                )
                              )
                                return;
                              submitFetcher({
          ...
          <div className="mt-4 max-w-2xl rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
            ⚠️ 카테고리 삭제 시, 해당 카테고리로 등록된 제품의 카테고리가 미분류
            상태가 됩니다. 삭제 전 제품의 카테고리를 먼저 변경해 주세요.
          </div>
        </div>
      </div>
