...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "create") {
    const maxOrder = await db.select({ sort_order: recipeCategories.sort_order })
  ...
  if (intent === "delete") {
    const id = Number(fd.get("id"));
    if (id) await db.delete(recipeCategories).where(eq(recipeCategories.category_id, id));
    return { success: true };
  }
  ...
  if (intent === "toggle") {
    ...
    if (id) await db.update(recipeCategories).set({ is_active: !isActive })
      .where(eq(recipeCategories.category_id, id));
    return { success: true };
  }
  ...
  if (intent === "reorder") {
    ...
    const all = await db.select().from(recipeCategories).orderBy(recipeCategories.sort_order);
    ...
    if (idx >= 0 && swapIdx >= 0 && swapIdx < all.length) {
      ...
      await db.update(recipeCategories).set({ sort_order: bOrder }).where(eq(recipeCategories.category_id, a.category_id));
      await db.update(recipeCategories).set({ sort_order: aOrder }).where(eq(recipeCategories.category_id, b.category_id));
    }
    return { success: true };
  }
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              required
            />
            <p className="text-xs text-gray-400">레시피 카테고리 필터에 사용되는 고유 식별자입니다.</p>
          </div>
...
export default function AdminRecipeCategoriesPage({ loaderData }: Route.ComponentProps) {
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-8">
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">레시피 카테고리 관리</h1>
          ...
          <div className="max-w-2xl overflow-hidden rounded-lg bg-white shadow">
            ...
            {categories.length === 0 ? (
              <div className="py-12 text-center text-gray-400">
                <p>등록된 카테고리가 없습니다.</p>
                <p className="mt-1 text-sm">카테고리를 추가하면 레시피 페이지 탭에 표시됩니다.</p>
              </div>
            ) : (
          ...
          <div className="mt-4 max-w-2xl rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
            ⚠️ 카테고리 삭제 시, 해당 카테고리로 등록된 레시피가 미분류 상태가 됩니다. 삭제 전 레시피의 카테고리를 먼저 변경해 주세요.
          </div>
        </div>
      </div>
